...
DefaultPriceCriterion::DefaultPriceCriterion(double standardDeviation_,double sampleAverage_){
    standardDeviation = standardDeviation_;
    sampleAverage = sampleAverage_;
}


vector<Hotel*> DefaultPriceCriterion::applyFilter(std::vector<Hotel*> hotels){
    vector<Hotel*> filteredHotels;
    for(int i=0;i<hotels.size();i++){
        if(abs(hotels[i]->getAveragePrice() - sampleAverage) <= 2*standardDeviation){
            filteredHotels.push_back(hotels[i]);
        }
    }
    return filteredHotels;
