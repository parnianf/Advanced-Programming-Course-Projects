...
Utrip::Utrip(string hotelFilePath,string ratingFilePath){
    setInitialFileData(cvsReader.readHotelData(hotelFilePath),cvsReader.readRatingData(ratingFilePath));
    currentUser = NULL;
    didSortTheHotels = true;
}
...
void Utrip::setInitialFileData(vector<Hotel*> initialHotels , map<string,Rating> ratings){
    hotels = initialHotels;
    for(auto rating : ratings){
        findHotelById(rating.first)->setPreviousRatings(rating.second);
    }

}

void Utrip::showEstimatedWeight(){
    currentUser->showEstimatedWeight();
}

void Utrip::showManualWeight(){
    currentUser->showManualWeight();
}


void Utrip::setManualWeights(Arguments arg){
    currentUser->setManualWeight(Rating(stod(arg[LOCATION].c_str()),stod(arg[CLEANLINESS].c_str()),
                                stod(arg[STAFF].c_str()),stod(arg[FACILITIES].c_str()),stod(arg[VALUE_FOR_MONEY].c_str()),1));
}

void Utrip::sortHotels(Arguments args){
    Sort sortHotels;
    string property = args[PROPERTY];
    hotels = sortHotels.sortInOrder(hotels,args[PROPERTY],args[ORDER],currentUser,didSortTheHotels);
}
...
void Utrip::setCurrentUser(string email,string password){
    for(int i=0;i<users.size();i++){
        if(users[i]->doesExist(email,password)){
            currentUser = users[i];
            currentUser->turnOnorOffDefPriceFilter(true);
            cout<<SUCCESSFUL<<endl;
            return;
        }
    }
...
void Utrip::logoutUser(){
    currentUser->deactivateManualWeight();
    deleteFilters();
    currentUser = NULL;
}

void Utrip::deactivateManualWeght(){
    currentUser->deactivateManualWeight();
}
...
void Utrip::login(Arguments info){
    ...
    if(isUserValid(info[EMAIL],to_string(hashedPassword))){
        ...
        currentUser->turnOnorOffDefPriceFilter(true);
        hotels = sortHotels.sortInOrder(hotels,ID,ASCENDING,currentUser,didSortTheHotels);
        return;
    }
    throw BadRequest();
}
...
void Utrip::reserve(string hotelId,RoomType roomType,int quantity,Duration duration){
    ...
    currentUser->getStandardDeviation();
}
...
void Utrip::updatePriceFilter(){
    currentUser->applyDefaultPriceFilter();
}

void Utrip::turnDefPriceFilter(string active_){
    if(active_ != TRUE_ && active_ != FALSE_)
        throw BadRequest();
    bool active = active_ == TRUE_ ? true : false;
    currentUser->turnOnorOffDefPriceFilter(active);
    cout<<SUCCESSFUL<<endl;
}
...
void Utrip::addRating(Arguments arg){
    ...
    Rating rating(stod(arg[LOCATION].c_str()),stod(arg[CLEANLINESS].c_str()),stod(arg[STAFF].c_str()),
