...
void ConsoleHandler::processCommand(){
    ...
    while(getline(cin,line)){
        ...
        getline(data,commandType,ARGUMENTS_SEPERATOR);
        updateLogin();
        try{
            if(commandType == POST)
...
void ConsoleHandler::parseDeleteOreders(stringstream& data){
    ...
    getline(data,command,ARGUMENTS_SEPERATOR);
    int qMark = data.str().find(ORDER_ARGUMENTS_SEPERATOR);
    if(qMark != string::npos)
        getline(data,questionMark,ARGUMENTS_SEPERATOR); 
    if(!isLoggedIn)
...
void ConsoleHandler::parsePostOrders(stringstream& data){
    ...
    if(!isInOrder(command,data))
        return;
    doPostOrders(command,data);
}

void ConsoleHandler::doPostOrders(string command,stringstream& data){
    if(command == LOGOUT)
        logout();
        ...
        applyDefaultFilter(data);
    else if(command == SORT)
        postSort(data);
    else if(command == MANUAL_WEIGHT)
        postManualWeight(data);
    else 
        throw NotFound();
}
...
void ConsoleHandler::parseGetOrders(stringstream& data){
    ...
    else if(command == ESTIMATED_WEIGHT){
        utrip->showEstimatedWeight();
    }
    else 
        throw NotFound();
...
void ConsoleHandler::postSort(stringstream& data){
    Arguments arguments = parseInput(data,getPostArgumentTypes(SORT));
    if(isValid(arguments)){
        utrip->sortHotels(arguments);
        cout<<SUCCESSFUL<<endl;
    }
}
...
void ConsoleHandler::postFilter(stringstream& info){
    ...
    getline(ss,word,ARGUMENTS_SEPERATOR);
    if(word == CITY)
        filter = "cityFilter";
...
void ConsoleHandler::applyDefaultFilter(stringstream& data){
    Arguments arguments = parseInput(data,{ACTIVE});
    if(isValid(arguments)){
        utrip->turnDefPriceFilter(arguments[ACTIVE]);
    }
}
...
void ConsoleHandler::getManualWeight(){
    utrip->showManualWeight();
}
...
void ConsoleHandler::postManualWeight(stringstream& data){
    if((data.str().find(FALSE_) == string::npos) && (data.str().find(TRUE_) == string::npos))
        throw BadRequest();
    if(data.str().find(FALSE_) != string::npos){
        Arguments arguments = parseInput(data,{ACTIVE});
        if(arguments[ACTIVE] != FALSE_)
            throw BadRequest();
        utrip->deactivateManualWeght();
        cout<<SUCCESSFUL<<endl;
        return;
    }
    Arguments arguments = parseInput(data,getPostArgumentTypes(MANUAL_WEIGHT));
    if(isValid(arguments)){
        utrip->setManualWeights(arguments);
        cout<<SUCCESSFUL<<endl;
    }
}
...
vector<string> ConsoleHandler::getPostArgumentTypes(string command){
    if(command == SIGNUP){
        ...
        vector<string> arguments{ PROPERTY , ORDER};
        return arguments;
    }
    else if(command == MANUAL_WEIGHT){
        vector<string> arguments { ACTIVE,LOCATION,CLEANLINESS,STAFF,FACILITIES,VALUE_FOR_MONEY};
        return arguments;
    }
...
Arguments ConsoleHandler::parseInput(stringstream& data,vector<string> types){
    ...
    for(int i = 0;i<size;i++){
        getline(data,type,ARGUMENTS_SEPERATOR);
        for(int j = 0 ; j < size ; j++){
            if(type == types[j]){
                ...
                getline(data,arg,ARGUMENTS_SEPERATOR);
                arguments[type] = arg; 
            }
        }
    }
